...
class GetTotalAssets(Resource):
    def get(self):
        ...
        total_usd = 0
        for _, v in token_stats.items():
            if "current_price" in v.keys() and v["current_price"] is not None:
                total_usd += v["current_price"] * v["count"]
            else:
                v["current_price"] = 0
                v["count"] = 0
        
        token_stats = dict(sorted(token_stats.items(), key=lambda x: x[1]["current_price"] * x[1]["count"], reverse=True))
...
def main():
    print(f"using port {APP_CONFIG.BACKEND_API_PORT}")
